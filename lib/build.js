...
init = function() {
    ...
    for (var from in tsconfig.compilerOptions.paths) {
        for (var i = 0; i < tsconfig.compilerOptions.paths[from].length; i++) {
            var to = tsconfig.compilerOptions.paths[from][i];
            pathModify.mods.push(pathmodify.mod.id(from, process.cwd() + '/' + inputDirectoryName + '/' + to));
        }
    }
